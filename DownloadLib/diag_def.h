...
/* 74 Reserved */
#if defined(LGE_FW_ANT_DIAG_CMD)
#define DIAG_ANT_ADC_RD_F   74
#endif
...
/* 76-80 Reserved */
#if defined(LGE_FW_GPS_DIAG_CMD)
#define DIAG_GNSS_DRE_DATA_RD_F   77
#endif
...
/* Gen11 MICOM update packet */
#define DIAG_MICOM_F		208
#define MICOM_FIRST_UPDATE	((char)1)	// invoke micom first update (Update partition under NORMAL)
#define MICOM_NORMAL_TO_RR	((char)2)	// invoke micom mode to RR
#define MICOM_SECOND_UPDATE	((char)3)	// invoke micom second update (Normal partition under RR)
#define MICOM_RR_TO_NORMAL	((char)4)	// invoke micom mode to NORMAL
#define MICOM_READ_MODE		((char)5)	// read current micom's mode
#define MICOM_ALL_UPDATE	((char)6)	// invoke micom all update
#define MICOM_MKEY_ERASE	((char)11)	// jwoh add GB key erase function
#define MICOM_MKEY_ERASE_CHECK	((char)12)	// jwoh add GB key erase check function
#define MICOM_RESET			((char)16)	// micom reset
#define MICOM_CMD_END		((char)0)   // the command  step is end
#define MICOM_UPDATE_STATUS	((char)127)	// check command processing status
#define MICOM_RESULT_STATUS	((char)128)	// read result status

/* Gen11 MDM debug mode change */
#define DIAG_DEBUG_MODE_F		209
#define MDM_DEBUGMODE_ON		((char)1)
#define MDM_DEBUGMODE_OFF		((char)2)
#define MDM_PLATFORMID_CHECK	((char)3) // jwoh platfom id check
#define MDM_DEBUGMODE_STATUS	((char)127)

/* Gen11 extended request packet for downloader */
#define DIAG_DOWNLOAD_EXT_F   253

#define DIAG_RESET_F		  41
#define DIAG_REBOOT_NORMAL    ((char)2)
#ifdef LG_FW_DIAG_CMD
#define DIAG_ANTENNA_SWITCHING_TEST			252
#endif
...
typedef enum {
  ...
  DIAG_SUBSYS_EFSBACKUP			 = 55,      /* EFS BACKUP  */
...
typedef struct {
  byte               cmd_code;
  word               mode;
} diag_control_req_type;  // cmdiag_mode_enum_type
...
/*SYS: DIAG_DOWNLOAD_EXT_F
  SUB: DIAG_SUBSYS_DOWNLOAD
*/
#define VERNO_HW_REV_STRLEN 15
#define VERNO_MCFG_VER_STRLEN   10

typedef struct
{
	byte cmd_code;
	byte response;
	char hw_rev[VERNO_HW_REV_STRLEN];
	char mcfg_ver[VERNO_MCFG_VER_STRLEN];
	unsigned int hw_area;
} diag_download_get_ver_rsp_type;

#define DOWNLOAD_REBOOT		1
#define DOWNLOAD_VERSION	2
#define DOWNLOAD_NRESET		3

typedef struct 
{
	byte cmd_code;
	byte subsys_id;
	unsigned short sybsys_code;
	unsigned int status;
	unsigned short delayed_rsp_id;
	unsigned short resp_count;
} diag_efs_backup_resp_type;
